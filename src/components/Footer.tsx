...
export function Footer() {
  ...
  const tabs = [
    ...
    { path: "/admin", icon: <FaUserShield size={24} />, label: "Админ" },
  ];
        ...
        {tabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.path}
              to={tab.path}
              className={`flex flex-col items-center transition-all active:scale-95 ${isActive ? 'text-green-400' : 'text-zinc-400'}`}
            >
              {tab.icon}
